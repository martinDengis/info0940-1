...
static int buffer_out(void *ctx, u32 pid, char *comm, u64 page_fault_count)
{
    // create the out object following `page_fault_event_out` structure
    struct page_fault_event_out out_this = {
        .timestamp = bpf_ktime_get_ns(),
        .pid = pid,
        .page_fault_count = page_fault_count
    };
    bpf_probe_read_str(out_this.comm, sizeof(out_this.comm), comm);
               ...
               unsigned int flags, struct pt_regs *regs)
{
    ...
    if (ft == NULL) {
        ...
        if (log_step == 1)
        {
            buffer_out(ctx, pid, new_ft.comm, 1);
        }
    } else {
        ...
        {
            buffer_out(ctx, pid, ft->comm, ft->pg_fault_count);
        }
    }
